 ...
 */       
void
FillWHX(int *s, int len, int **icfg, int **wbx, int **vx, 
	 ...
	 int j, int d, int d1, int d2)
{
  ...
  /* (WH2.1) */
  if ((sc = P10P + zhx[j][d][d1][d2]) > bestsc)
    bestsc = sc;
  ...
  /* (WH3.1) */
  if ((sc = P10P + yhx[j][d][d1][d2]) > bestsc)
    bestsc = sc;
  ...
  /* (WH4.1) */
  if (d1 > 0 &&
      (sc = P6P + whx[j][d-1][d1-1][d2] ) > bestsc)
    bestsc = sc;
  
  /* (WH4.2) */
  if (d2 > 0 &&
      (sc = P6P + whx[j-1][d-1][d1][d2-1]) > bestsc)
    bestsc = sc;
  
  /* (WH4.3) */ 
  if (d2 > 0 &&
      (sc = P6P + whx[j][d][d1][d2-1]) > bestsc)
    bestsc = sc;
  
  /* (WH4.4) */
  if (d1 > 0 &&
      (sc = P6P + whx[j][d][d1-1][d2]) > bestsc)
    bestsc = sc;
 ...
 */ 
void
TraceWHX(FILE *outf, int *s, int len, int **icfg, int **wbx, int **vx, 
	 int ****whx, int ****vhx, int ****zhx, int ****yhx,
	 int j, int d, int d1, int d2, 
	 struct tracekn_s *curr_tr, struct traceknstack_s *dolist, int traceback)
{
   ...
   */ 
  /* (WH1.1) */
  if (whx[j][d][d1][d2] == WH1_1(s, len, icfg, vhx, j, d, d1, d2)){
    trace_WH1_1(outf, vhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }   
  
  /* (WH1.2) */
  else if (whx[j][d][d1][d2] == WH1_2(s, len, icfg, vhx, j, d, d1, d2)){
    trace_WH1_2(outf, vhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }   
  
  /* (WH1.3) */
  else if (whx[j][d][d1][d2] == WH1_3(s, len, icfg, vhx, j, d, d1, d2)){ 
   trace_WH1_3(outf, vhx, j, d, d1, d2, curr_tr, dolist, traceback);
   return;
  }
  
  /* (WH1.4) */
  else if (whx[j][d][d1][d2] == WH1_4(s, len, icfg, vhx, j, d, d1, d2)){
    trace_WH1_4(outf, vhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH1.5) */
  if (whx[j][d][d1][d2] == WH1_5(s, len, icfg, vhx, j, d, d1, d2)){    
    trace_WH1_5(outf, vhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH1.6) */
  else if (whx[j][d][d1][d2] == WH1_6(s, len, icfg, vhx, j, d, d1, d2)){ 
    trace_WH1_6(outf, vhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH1.7) */
  else if (whx[j][d][d1][d2] == WH1_7(s, len, icfg, vhx, j, d, d1, d2)){ 
    trace_WH1_7(outf, vhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH1.8) */
  else if (whx[j][d][d1][d2] == WH1_8(s, len, icfg, vhx, j, d, d1, d2)){ 
    trace_WH1_8(outf, vhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH1.9) */
  else if (whx[j][d][d1][d2] == WH1_9(s, len, icfg, vhx, j, d, d1, d2)){ 
    trace_WH1_9(outf, vhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH1.10) */
  else if (whx[j][d][d1][d2] == WH1_10(s, len, icfg, vhx, j, d, d1, d2)){
   ...
   */ 
  /* (WH2.1) */
  else if (whx[j][d][d1][d2] == WH2_1(s, len, icfg, zhx, j, d, d1, d2)){ 
    trace_WH2_1(outf, zhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH2.2) */
  else if (whx[j][d][d1][d2] == WH2_2(s, len, icfg, zhx, j, d, d1, d2)){ 
    trace_WH2_2(outf, zhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH2.3) */
  else if (whx[j][d][d1][d2] == WH2_3(s, len, icfg, zhx, j, d, d1, d2)){ 
    trace_WH2_3(outf, zhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH2.4) */
  else if (whx[j][d][d1][d2] == WH2_4(s, len, icfg, zhx, j, d, d1, d2)){ 
   ...
   */ 
  /* (WH3.1) */
  else if (whx[j][d][d1][d2] == WH3_1(s, len, icfg, yhx, j, d, d1, d2)){ 
    trace_WH3_1(outf, yhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH3.2) */
  else if (whx[j][d][d1][d2] == WH3_2(s, len, icfg, yhx, j, d, d1, d2)){ 
    trace_WH3_2(outf, yhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }

  /* (WH3.3) */
  else if (whx[j][d][d1][d2] == WH3_3(s, len, icfg, yhx, j, d, d1, d2)){
    trace_WH3_3(outf, yhx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH3.4) */
  else if (whx[j][d][d1][d2] == WH3_4(s, len, icfg, yhx, j, d, d1, d2)){ 
   ...
   */ 
  /* (WH4.1) */
  else if (whx[j][d][d1][d2] == WH4_1(s, len, icfg, whx, j, d, d1, d2)){ 
    trace_WH4_1(outf, whx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH4.2) */
  else if (whx[j][d][d1][d2] == WH4_2(s, len, icfg, whx, j, d, d1, d2)){
    trace_WH4_2(outf, whx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH4.3) */ 
  else if (whx[j][d][d1][d2] == WH4_3(s, len, icfg, whx, j, d, d1, d2)){
    trace_WH4_3(outf, whx, j, d, d1, d2, curr_tr, dolist, traceback);
    return;
  }
  
  /* (WH4.4) */
  else if (whx[j][d][d1][d2] == WH4_4(s, len, icfg, whx, j, d, d1, d2)){ 
  ...
  /* (WH5)__/ 2 WBX  
   */
  else if (whx[j][d][d1][d2] == WH5(s, len, icfg, wbx, j, d, d1, d2)){
  ...
  for (mid = d1-1; mid > 0; mid--) {
    ...
    /* (WH6)__/  1 WBX, 1 WHX
     */                                       
    if (whx[j][d][d1][d2] == WH6(s, len, icfg, wbx, whx, j, d, d1, d2, mid)){ 
     ...
     */                                       
    /* (WH7.1) */                                   
    else if (whx[j][d][d1][d2] == WH7_1(s, len, icfg, vx, zhx, j, d, d1, d2, mid)){ 
      trace_WH7_1(outf, vx, zhx, j, d, d1, d2, mid, curr_tr, dolist, traceback);
      return;
    }
	  
    /* (WH7.2) */                                   
    else if (whx[j][d][d1][d2] == WH7_2(s, len, icfg, vx, zhx, j, d, d1, d2, mid)){ 
      trace_WH7_2(outf, vx, zhx, j, d, d1, d2, mid, curr_tr, dolist, traceback);
      return;
    }
	  
    /* (WH7.3) */                                   
    else if (whx[j][d][d1][d2] == WH7_3(s, len, icfg, vx, zhx, j, d, d1, d2, mid)){ 
      trace_WH7_3(outf, vx, zhx, j, d, d1, d2, mid, curr_tr, dolist, traceback);
      return;
    }
	  
    /* (WH7.4) */                                   
    else if (whx[j][d][d1][d2] == WH7_4(s, len, icfg, vx, zhx, j, d, d1, d2, mid)){ 
    ...
    /* (WH8)__/ 1 WBX 1 WHX
     */                                       
    else if (whx[j][d][d1][d2] == WH8(s, len, icfg, wbx, whx, j, d, d1, d2, mid)){  
     ...
     */
    /* (WH9.1) */                                   
    else if (whx[j][d][d1][d2] == WH9_1(s, len, icfg, vx, vhx, j, d, d1, d2, mid)){  
      trace_WH9_1(outf, vx, whx, j, d, d1, d2, mid, curr_tr, dolist, traceback);
      return;
    }

    /* (WH9.2) */                                   
    else if (whx[j][d][d1][d2] == WH9_2(s, len, icfg, vx, vhx, j, d, d1, d2, mid)) { 
      trace_WH9_2(outf, vx, whx, j, d, d1, d2, mid, curr_tr, dolist, traceback);
      return;
    }

    /* (WH9.3) */                                   
    else if (whx[j][d][d1][d2] == WH9_3(s, len, icfg, vx, vhx, j, d, d1, d2, mid)){  
      trace_WH9_3(outf, vx, whx, j, d, d1, d2, mid, curr_tr, dolist, traceback);
      return;
    }

    /* (WH9.4) */                                   
    else if (whx[j][d][d1][d2] == WH9_4(s, len, icfg, vx, vhx, j, d, d1, d2, mid)){   
  ...
  for (mid = d2 - 1; mid > 0; mid--) {
    ...
    /* (WH10)__/ 1 WBX 1 WHX
     */                                       
    if (whx[j][d][d1][d2] == WH10(s, len, icfg, wbx, whx, j, d, d1, d2, mid)){ 
     ...
     */                                       
    /* (WH11.1) */                                   
    else if (whx[j][d][d1][d2] == WH11_1(s, len, icfg, vx, zhx, j, d, d1, d2, mid)){ 
      trace_WH11_1(outf, vx, zhx, j, d, d1, d2, mid, curr_tr, dolist, traceback);
      return;
    }
      
    /* (WH11.2) */                                   
    else if (whx[j][d][d1][d2] == WH11_2(s, len, icfg, vx, zhx, j, d, d1, d2, mid)){ 
      trace_WH11_2(outf, vx, zhx, j, d, d1, d2, mid, curr_tr, dolist, traceback);
      return;
    }
      
    /* (WH11.3) */                                   
    else if (whx[j][d][d1][d2] == WH11_3(s, len, icfg, vx, zhx, j, d, d1, d2, mid)){ 
      trace_WH11_3(outf, vx, zhx, j, d, d1, d2, mid, curr_tr, dolist, traceback);
      return;
    }
      
    /* (WH11.4) */                                       
    else if (whx[j][d][d1][d2] == WH11_4(s, len, icfg, vx, zhx, j, d, d1, d2, mid)){ 
    ...
    /* (WH12)__/ 1 WBX 1 WHX
     */                                       
    else if (whx[j][d][d1][d2] == WH12(s, len, icfg, wbx, whx, j, d, d1, d2, mid)){     
     ...
     */ 
    /* (WH13.1) */                                   
    else if (whx[j][d][d1][d2] == WH13_1(s, len, icfg, vx, yhx, j, d, d1, d2, mid)){     
      trace_WH13_1(outf, vx, yhx, j, d, d1, d2, mid, curr_tr, dolist, traceback);
      return;
    }
      
    /* (WH13.2) */                                   
    else if (whx[j][d][d1][d2] == WH13_2(s, len, icfg, vx, yhx, j, d, d1, d2, mid)){     
      trace_WH13_2(outf, vx, yhx, j, d, d1, d2, mid, curr_tr, dolist, traceback);
      return;
    }
      
    /* (WH13.3) */                                   
    else if (whx[j][d][d1][d2] == WH13_3(s, len, icfg, vx, yhx, j, d, d1, d2, mid)){     
      trace_WH13_3(outf, vx, yhx, j, d, d1, d2, mid, curr_tr, dolist, traceback);
      return;
    }
      
    /* (WH13.4) */                                       
    else if (whx[j][d][d1][d2] == WH13_4(s, len, icfg, vx, yhx, j, d, d1, d2, mid)){     
  ...
  for ((mid1 = d1 - 1); (mid1 >= 0); mid1--)
    for ((mid2 = d2 - 1); (mid2 >= 0); mid2--) {
      ...
      /* (WH14)__/ STRUCTURE WITH ONE YHX AND ONE ZHX. (only one pair)
       */ 
      if (whx[j][d][d1][d2] == WH14(s, len, icfg, zhx, yhx, j, d, d1, d2, mid1, mid2)){    
	...
	return;
      ...
      /* (WH15)__/  2 WHX (inclusive bifurcation)
       */
      else if (whx[j][d][d1][d2] == WH15(s, len, icfg, whx, j, d, d1, d2, mid1, mid2)){    
	...
	return;
      ...
      /* (WH16)__/  2 WHX (crossed bifurcation)
       */
      else if (whx[j][d][d1][d2] == WH16(s, len, icfg, whx, j, d, d1, d2, mid1, mid2)){    
  ...
  for ((mid1 = d2 - 2); (mid1 >= 0); mid1--) 
    for (mid2 = d2 - mid1 - 2; mid2 >= 0; mid2--) {
      ...
      /* (WH17)__/  2 WHX 
       */
      if (whx[j][d][d1][d2] == WH17(s, len, icfg, whx, j, d, d1, d2, mid1, mid2)){         
	...
	return;
       ...
       */  
      /* (WH18.1) */                                   
      else if (whx[j][d][d1][d2] == WH18_1(s, len, icfg, zhx, yhx, j, d, d1, d2, mid1, mid2)){ 
	trace_WH18_1(outf, zhx, yhx, j, d, d1, d2, mid1, mid2, curr_tr, dolist, traceback);
	return;
      }
      
      /* (WH18.2) */
      else if (whx[j][d][d1][d2] == WH18_2(s, len, icfg, zhx, yhx, j, d, d1, d2, mid1, mid2)){  
       	trace_WH18_2(outf, zhx, yhx, j, d, d1, d2, mid1, mid2, curr_tr, dolist, traceback);
	return;
      }

      /* (WH18.3) */
      else if (whx[j][d][d1][d2] == WH18_3(s, len, icfg, zhx, yhx, j, d, d1, d2, mid1, mid2)){
	trace_WH18_3(outf, zhx, yhx, j, d, d1, d2, mid1, mid2, curr_tr, dolist, traceback);
	return;
      }
      
      /* (WH18.4) */
      else if (whx[j][d][d1][d2] == WH18_4(s, len, icfg, zhx, yhx, j, d, d1, d2, mid1, mid2)){     
	...
	return;
       ...
       */
      /* (WH19.1) */                                   
      else if (whx[j][d][d1][d2] == WH19_1(s, len, icfg, yhx, j, d, d1, d2, mid1, mid2)){        
	trace_WH19_1(outf, yhx, j, d, d1, d2, mid1, mid2, curr_tr, dolist, traceback);
	return;
      }
	  
      /* (WH19.2) */
      else if (whx[j][d][d1][d2] == WH19_2(s, len, icfg, yhx, j, d, d1, d2, mid1, mid2)){         
	trace_WH19_2(outf, yhx, j, d, d1, d2, mid1, mid2, curr_tr, dolist, traceback);
      }

      /* (WH19.3) */
      else if (whx[j][d][d1][d2] == WH19_3(s, len, icfg, yhx, j, d, d1, d2, mid1, mid2)){         
	trace_WH19_3(outf, yhx, j, d, d1, d2, mid1, mid2, curr_tr, dolist, traceback);
	return;
      }
	  
      /* (WH19.4) */
      else if (whx[j][d][d1][d2] == WH19_4(s, len, icfg, yhx, j, d, d1, d2, mid1, mid2)){       
  ...
  for ((mid1 = d1 - 2); (mid1 >= 0); mid1--) 
    for ((mid2 = d1 - mid1 - 2); (mid2 >= 0); mid2--) {
      ...
      /* (WH20)__/  2 WHX */
      if (whx[j][d][d1][d2] == WH20(s, len, icfg, whx, j, d, d1, d2, mid1, mid2)){        
	...
	return;
       ...
       */
      /* (WH21.1) */                                   
      else if (whx[j][d][d1][d2] == WH21_1(s, len, icfg, zhx, yhx, j, d, d1, d2, mid1, mid2)){
	trace_WH21_1(outf, zhx, yhx, j, d, d1, d2, mid1, mid2, curr_tr, dolist, traceback);
	return;
      }

      /* (WH21.2) */
      else if (whx[j][d][d1][d2] == WH21_2(s, len, icfg, zhx, yhx, j, d, d1, d2, mid1, mid2)){
	trace_WH21_2(outf, zhx, yhx, j, d, d1, d2, mid1, mid2, curr_tr, dolist, traceback);
	return;
      }

      /* (WH21.3) */
      else if (whx[j][d][d1][d2] == WH21_3(s, len, icfg, zhx, yhx, j, d, d1, d2, mid1, mid2)){
	trace_WH21_3(outf, zhx, yhx, j, d, d1, d2, mid1, mid2, curr_tr, dolist, traceback);
	return;
      }
      
      /* (WH21.4) */
      else if (whx[j][d][d1][d2] == WH21_4(s, len, icfg, zhx, yhx, j, d, d1, d2, mid1, mid2)){
	...
	return;
       ...
       */
      /* (WH22.1) */                                   
      else if (whx[j][d][d1][d2] == WH22_1(s, len, icfg, yhx, j, d, d1, d2, mid1, mid2)){        
	trace_WH22_1(outf, yhx, j, d, d1, d2, mid1, mid2, curr_tr, dolist, traceback);
	return;
      }

      /* (WH22.2) */
      else if (whx[j][d][d1][d2] == WH22_2(s, len, icfg, yhx, j, d, d1, d2, mid1, mid2)){        
	trace_WH22_2(outf, yhx, j, d, d1, d2, mid1, mid2, curr_tr, dolist, traceback);
	return;
      }

      /* (WH22.3) */
      else if (whx[j][d][d1][d2] == WH22_3(s, len, icfg, yhx, j, d, d1, d2, mid1, mid2)){        
	trace_WH22_3(outf, yhx, j, d, d1, d2, mid1, mid2, curr_tr, dolist, traceback);
	return;
      }

      /* (WH22.4) */
      else if (whx[j][d][d1][d2] == WH22_4(s, len, icfg, yhx, j, d, d1, d2, mid1, mid2)){        
