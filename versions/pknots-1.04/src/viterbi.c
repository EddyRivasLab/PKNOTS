 ...
 */          
void
StructurePredictkn_2IS(FILE *outf, char *seq, int len, int **icfg,  
		       ...
		       int *ret_score, int allow_coaxials, int allow_pseudoknots, int approx)
{
 ...
 if (allow_pseudoknots) {
    ...
    FillMtx(iseq, len, icfg,  wx, wbx, vx, vp, whx, vhx, zhx, yhx, allow_coaxials, approx);
  }
  else
    FillMtx_nested(iseq, len, icfg,  wx, wbx, vx, vp, allow_coaxials);
  ...
  */
  ...
  if (allow_pseudoknots) 
    TraceMtx(outf, iseq, len, icfg, wx, wbx, vx,  whx, vhx, zhx, yhx,
	     len-1, len-1, (int)((len-1)/2), len-(int)((len-1)/2)-2,  
	     approx, &tr, traceback);
 else
    TraceMtx_nested(outf, iseq, len, icfg, wx, wbx, vx, len-1, len-1, &tr, traceback);
 ...
 */
void
Print_2DMtx(FILE *fp, int **mtx, int len)
{
  int j, d;
  ...
  for (j = 0; j < len; j++)
    {
      ...
      for (d = 0; d <= j; d++)
             if (mtx[j][d] == -123456789) /* pattern */
               fprintf(fp, "%9s ", "..");
        else if (mtx[j][d] <= (d+1) * P6)  /* prohibited */
               fprintf(fp, "%9s ", "**");
        else
 ...
 */
void
Print_4DMtx(FILE *fp, int ****mtx, int len)
{
  int j, d, d1, d2;
      ...
      for (d1 = 0; d1 <= d; d1++)
        {
          ...
          for (d2 = 0; d2 <= (d - d1); d2++)
		     ...
		     mtx[j][d][d1][d2] <= (d + 1) * P6) /* prohibited */
	           fprintf(fp, "%9s ", "**");
	    else if (d1 + d2 == d - 1 && 
		     mtx[j][d][d1][d2] <= (d1 + d2 + 1) * P6) /* prohibited */
	           fprintf(fp, "%9s ", "**");
	    else if (mtx[j][d][d1][d2] <= (d1+d2+2) * P6P) /* prohibited */
    	           fprintf(fp, "%9s ", "**");
	    else
