...
static void param_output(FILE *outf, int format, int shuffleseq, int allow_pseudoknots, 
...
{ 
  ...
  while (Getopt(argc, argv, OPTIONS, NOPTIONS, usage,
	...
	{
	  puts(banner); 
	  printf("         KNOT %s (%s)", RELEASE, RELEASEDATE);
  ...
  while (ReadSeq(sqfp, format, &seq, &sqinfo))
	...
	printf("\n");
     ...
     if (shuffleseq) ShuffleSequence(seq, sqinfo.len, sqinfo.len-1, verbose);
      ...
      StructurePredictkn_2IS(ofp, seq, sqinfo.len, icfg, verbose, traceback,
	...
	CompareRNAStructures(ofp, 0, sqinfo.len, sqinfo.ss, ss);
      ...
      param_output(ofp, format, shuffleseq, allow_pseudoknots, approx, score, pairs, cykpairs);
 ...
 */          
static void
param_output(FILE *ofp, int format, int shuffleseq, int allow_pseudoknots, int approx, 
