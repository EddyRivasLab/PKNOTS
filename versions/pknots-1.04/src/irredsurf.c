 ...
 */       
int
F1(int *s, int len, int **icfg, int j, int d)
{
  int F1;
  ...
  if (icfg[idxS][idxP(s[i],s[j])] == 1*INTSCALE)
	...
	F1 = EPARAM4 
	  + icfg[idxPL(s[i],s[j])][idxPL(s[i+1],s[j-1])] 
	  + icfg[idxS][idxX(30)] 
	  + (int)(PRELOG*log((d-1)/30.));
	...
	F1 = EPARAM4 
	  + icfg[idxPL(s[i],s[j])][idxPL(s[i+1],s[j-1])] 
	  + icfg[idxS][idxX(d-1)]
	  + icfg[idxS][idxTL(s[i]+s[i+1]*4+s[i+2]*4*4+s[i+3]*4*4*4+s[i+4]*4*4*4*4+s[i+5]*4*4*4*4*4)];
      else
	F1 = EPARAM4 + icfg[idxPL(s[i],s[j])][idxPL(s[i+1],s[j-1])] 
	  + icfg[idxS][idxX(d-1)];
    }
 ...
 */       
int
F2(int *s, int len, int **icfg, int j, int d, int d1, int d2)
{
  int F2;
  int i;			/* sequence start position                         */
  ...
  if (icfg[idxS][idxP(s[i],s[j])] == 1*INTSCALE
    ...
    { 
      ...
      if (d1 == 1 && d2 == 1)
	F2 = EPARAM1 
	  + icfg[idxPS(s[i],s[j])][idxPS(s[k],s[l])];
	...
	F2 = EPARAM3 		
	  + icfg[idxPS(s[i],s[j])][idxPS(s[i+1],s[j-1])] 
	  + icfg[idxPS(s[l],s[k])][idxPS(s[l+1],s[k-1])];
	       ...
	       icfg[idxS][idxP(s[i+1],s[j-1])] == INTSCALE)
	{
	  if (icfg[idxPS(s[k-1],s[l+1])][idxPS(s[k],s[l])] ==  /* this is the case so far */
	    ...
	    F2 = EPARAM3 		
	      + icfg[idxPI(s[i],s[j])][idxPI(s[i+1],s[j-1])] 
	      + icfg[idxPI(s[l],s[k])][idxPI(s[l+1],s[k-1])];
	}
	...
	F2 = EPARAM3 
	  ...
	  + icfg[idxS][idxW(30)] 
	  /*+ penalty*/
	  + (int)(PRELOG*log((d1+d2-2)/30.));
      
      else 
	F2 = EPARAM3 
	  + icfg[idxPI(s[i],s[j])][idxPI(s[i+1],s[j-1])] 
	  + icfg[idxPI(s[l],s[k])][idxPI(s[l+1],s[k-1])] 
	  + icfg[idxS][idxW(d1+d2-2)]
	  /*+ penalty*/;
    }
  else F2 = -BIGINT;
