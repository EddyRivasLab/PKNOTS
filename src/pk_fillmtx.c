...
FillMtx_nested(ESL_DSQ *s, int len, struct rnapar_2 *rnapar, int **icfg, int **wx, int **wbx, int **vx, int *vp, int allow_coaxials)
{
  ...
  for (j = 0; j < len; j++)
      ...
      for (d = mind; d <= j; d++)        
	{
	   ...
	   *                      
	   */
	  FillVP(s, len, rnapar, icfg, vx, vp, j, d);
	...
	int ****whx, int ****vhx, int ****zhx, int ****yhx, int allow_coaxials, int approx)
{
  ...
  for (j = 0; j < len; j++)
      ...
      for (d = mind; d <= j; d++)        
	{
	   ...
	   */
	  FillVP(s, len, rnapar, icfg, vx, vp, j, d);
	  FillVX(s, len, rnapar, icfg, wbx, vx, vp, whx, zhx, yhx, j, d, allow_coaxials, approx);
	   ...
	   *                          and base pairs penalties (P10). Hole penalty (P13).
	   */
	  FillWX (s, len, rnapar, icfg, wx, vx, whx, yhx, j, d);
	  FillWBX(s, len, rnapar, icfg, wbx, vx, whx, yhx, j, d, approx);
	  ...
	  for (d1 = 0; d1 <= d; d1++)
	      ...
	      for (d2 = 0; d2 < (d - d1); d2++) 
		{
		   ...
		   */	 
		  FillVHX(s, len, rnapar, icfg, whx, vhx, j, d, d1, d2, min_kn);

		  /* filling  of ZHX(j)(d)(d1)(d2), (i,k)--(l,j). [i-j base-paired]
		   */
		  FillZHX(s, len, rnapar, icfg, wbx, vx, whx, vhx, zhx, j, d, d1, d2);

		  /* filling  of YHX(j)(d)(d1)(d2), (i,k)--(l,j). [k-l base-paired]
		   */
		  FillYHX(s, len, rnapar, icfg, wbx, vx, whx, vhx, yhx, j, d, d1, d2, min_kn);

		  /* filling  of WHX(j)(d)(d1)(d2), (i,k)--(l,j).
		   */
		  FillWHX(s, len, rnapar, icfg, wbx, vx, whx, vhx, zhx, yhx, j, d, d1, d2);
