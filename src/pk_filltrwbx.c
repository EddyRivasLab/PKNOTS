	...
	int ****whx,  int ****yhx, int j, int d, int approx)
{
   ...
   */
  FillWBX_nested(s, len, rnapar, icfg, wbx, vx, j, d);

  /* if approx = FALSE include internal pseudoknot  diagrams WB9-WB10. 
   */
  if (!approx) 
    FillWBX_pks(s, len, rnapar, icfg, wbx, vx, whx, yhx, j, d);
}
	 ...
	 struct tracekn_s *curr_tr, struct traceknstack_s *dolist, int traceback)
{
  ...
  if (!approx && *flag == FALSE){
    TraceWBX_pks(outf,s, len, rnapar, icfg, wbx, vx, whx, yhx, 
...
FillWBX_nested(ESL_DSQ *s, int len, struct rnapar_2 *rnapar, int **icfg, int **wbx, int **vx, int j, int d)
{
  ...
  /* (WB1)__/ PAIR (i,j).
   */
  bestsc = rnapar->P10 + vx[j][d];	

  /* (WB2)__/ PAIR (i+1,j-1).
   */
  if (d > 1 &&
      (sc = rnapar->P10 + 2*rnapar->P6 
   ...
   */
  if (d > 1 &&
      (sc = rnapar->P6 + wbx[j][d-1]) > bestsc)
    bestsc = sc;
  
  /* (WB6)__/  SINGLET-RIGHT.
   */
  if (d > 0 &&
      (sc = rnapar->P6 + wbx[j-1][d-1]) > bestsc)
    bestsc = sc; 
		...
		struct tracekn_s *curr_tr, struct traceknstack_s *dolist, int traceback)
{
  ...
  /* (WB1) */
  if (wbx[j][d] ==  WB1(s, len, rnapar, icfg, vx, j, d)){
  ...
  /* (WB2) */
  else if (wbx[j][d] == WB2(s, len, rnapar, icfg, vx, j, d)){
  ...
  /* (WB3) */
  else if (wbx[j][d] == WB3(s, len, rnapar, icfg, vx, j, d)){
  ...
  /* (WB4) */
  else if (wbx[j][d] == WB4(s, len, rnapar, icfg, vx, j, d)){
  ...
  /* (WB5) */
  else if (wbx[j][d] == WB5(s, len, rnapar, icfg, wbx, j, d)){
  ...
  /* (WB6) */
  else if (wbx[j][d] == WB6(s, len, rnapar, icfg, wbx, j, d)){
  ...
  for (mid = d-1; mid > 0; mid--) {
    /* (WB7) */         
    if (wbx[j][d] == WB7(s, len, rnapar, icfg, wbx, j, d, mid)){
    ...
    /* (WB8) */
    /* (WB8.1) */
    else if (wbx[j][d] == WB8_1(s, len, rnapar, icfg, vx, j, d, mid)){
    ...
    /* (WB8.2) */
    else if (wbx[j][d] == WB8_2(s, len, rnapar, icfg, vx, j, d, mid)){
    ...
    /* (WB8.3) */
    else if (wbx[j][d] == WB8_3(s, len, rnapar, icfg, vx, j, d, mid)){
    ...
    /* (WB8.4) */
    else if (wbx[j][d] == WB8_4(s, len, rnapar, icfg, vx, j, d, mid)){
    ...
    /* (WB8.5) */
    else if (wbx[j][d] == WB8_5(s, len, rnapar, icfg, vx, j, d, mid)){
    ...
    /* (WB8.6) */
    else if (wbx[j][d] == WB8_6(s, len, rnapar, icfg, vx, j, d, mid)){
    ...
    /* (WB8.7) */
    else if (wbx[j][d] == WB8_7(s, len, rnapar, icfg, vx, j, d, mid)){
    ...
    /* (WB8.8) */
    else if (wbx[j][d] == WB8_8(s, len, rnapar, icfg, vx, j, d, mid)){
	    ...
	    int ****whx, int ****yhx, int j, int d)
{
  ...
  for (mid = 3; mid < (d-2); mid++)            
	...
	{    
	   ...
	   */
	  /* (WB10.1) */
	  if ((sc = rnapar->P13 + 2*rnapar->P10P + 3*rnapar->P6P
	  ...
	  /* (WB10.2) */
	  if (mid-mid1-mid2 > 3 &&
	      (sc = rnapar->P13 + 2*rnapar->P10P + 4*rnapar->P6P
	       + wkn*icfg[idxR(s[i+mid1+1])][idxP(s[i+mid-mid2],s[i+mid1])]     
	     ...
	     int *flag, struct tracekn_s *curr_tr, struct traceknstack_s *dolist, int traceback)
{
  ...
  for (mid = d-3; mid >= 3; mid--)
	...
	/* (WB9)__/  2 WHX.
	 */
	if (wbx[j][d] == WB9(s, len, rnapar, icfg, whx, j, d, mid, mid1, mid2)){
	 ...
	 */
	/* (WB10.1) */
	else if (wbx[j][d] == WB10_1(s, len, rnapar, icfg, yhx, j, d, mid, mid1, mid2)){
	...
	/* (WB10.2) */
	else if (wbx[j][d] == WB10_2(s, len, rnapar, icfg, yhx, j, d, mid, mid1, mid2)){
